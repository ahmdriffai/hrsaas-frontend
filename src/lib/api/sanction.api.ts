...
} from "../model/sanction.model";
import type { Paging } from "../types/paging-types";

const BASE_URL = import.meta.env.VITE_API_PATH;

export const sanctionTypeCreate = async (
  token: string,
  data: z.infer<typeof CreateSanctionTypeSchema>
) => {
  return await fetch(`${BASE_URL}/sanctions`, {
    method: "POST",
    headers: {
...
export const sanctionCreate = async (
  token: string,
  request: z.infer<typeof CreateSanctionSchema>
) => {
  return await fetch(`${BASE_URL}/employee-sanctions`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Accept: "application/json",
